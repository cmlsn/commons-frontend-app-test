...
const WorkspaceSecureNotebookPage = ({
  ...
  // Initialize JupyterLab service manager after bootstrap completes
  useEffect(() => {
    const initServiceManager = async () => {
      console.log('[JEG-INIT-EFFECT] Starting service manager initialization...');
      ...
      while (!sm && attempts < maxAttempts) {
        // Try accessing from various globals (note: _JUPYTERLAB is uppercase!)
        sm = (window as any).jupyterlab?.serviceManager || 
             (window as any).jupyterapp?.serviceManager ||
             (window as any)._jupyterlab?.serviceManager ||
             (window as any)._JUPYTERLAB?.serviceManager ||  // Uppercase version!
             (window as any)._JUPYTERLAB?.app?.serviceManager ||
             (window as any)._JUPYTERLAB?.shell?.serviceManager;
        
        if (!sm && attempts % 10 === 0) {
          console.log(`[JEG-INIT-EFFECT] Attempt ${attempts + 1}: Service manager not found yet`);
        }
      ...
      if (!sm) {
        console.error('[JEG-INIT-EFFECT] Service manager not found after 10 seconds');
        const jupyterLikeKeys = Object.keys(window).filter((k: string) => 
          k.toLowerCase().includes('jupyter') || k.toLowerCase().includes('lab') || k.toLowerCase().includes('app')
        );
        console.log('[JEG-INIT-EFFECT] Jupyter-like globals:', jupyterLikeKeys);
        console.log('[JEG-INIT-EFFECT] Checking specific paths...');
        console.log('  - window.jupyterlab:', (window as any).jupyterlab);
        console.log('  - window._jupyterlab:', (window as any)._jupyterlab);
        console.log('  - window._JUPYTERLAB:', (window as any)._JUPYTERLAB);
        console.log('  - window._JUPYTERLAB?.app:', (window as any)._JUPYTERLAB?.app);
        console.log('  - window._JUPYTERLAB?.shell:', (window as any)._JUPYTERLAB?.shell);
        console.log('  - window._JUPYTERLAB?.serviceManager:', (window as any)._JUPYTERLAB?.serviceManager);
        
        // If we found _JUPYTERLAB but no serviceManager, explore its structure
        if ((window as any)._JUPYTERLAB) {
          console.log('  - _JUPYTERLAB keys:', Object.keys((window as any)._JUPYTERLAB));
          const coreOutput = (window as any)._JUPYTERLAB?.CORE_OUTPUT;
          if (coreOutput) {
            console.log('  - _JUPYTERLAB.CORE_OUTPUT type:', typeof coreOutput);
            console.log('  - _JUPYTERLAB.CORE_OUTPUT keys:', Object.keys(coreOutput).slice(0, 10));
          }
        }
        
        // Try finding JupyterLab app instance in DOM
        const jupyterSite = document.getElementById('jupyterlab-site');
        console.log('  - DOM #jupyterlab-site element:', jupyterSite);
        
        // Check if @datalayer/jupyter-react stores app in DOM
        const mainElement = document.getElementById('main');
        console.log('  - DOM #main element:', mainElement);
        if (mainElement) {
          console.log('  - #main dataset:', (mainElement as any).dataset);
          console.log('  - #main.__jupyterlab:', (mainElement as any).__jupyterlab);
        }
        
        // Last resort: scan all window properties for objects with serviceManager
        console.log('[JEG-INIT-EFFECT] Scanning window for serviceManager...');
        const keysWithServiceManager: string[] = [];
        for (const key of Object.keys(window)) {
          try {
            const val = (window as any)[key];
            if (val && typeof val === 'object' && val.serviceManager) {
              keysWithServiceManager.push(key);
              console.log(`  Found serviceManager at: window.${key}`);
            }
          } catch (e) {
            // Skip properties that throw on access
          }
        }
        if (keysWithServiceManager.length === 0) {
          console.log('  No window properties found with serviceManager');
        }
        
        return;
      }
      
      console.log('[JEG-INIT-EFFECT] Service manager found!', sm);
      (window as any).__jegServiceManager = sm;
      
      // Attempt to refresh kernel list
      try {
        console.log('[JEG-INIT-EFFECT] Refreshing running kernels...');
        await sm.kernels.refreshRunning();
        const running = Array.from((sm.kernels as any).running?.() || []);
        console.log('[JEG-INIT-EFFECT] Kernels after refresh:', running.length);
      } catch (err) {
        console.log('[JEG-INIT-EFFECT] Refresh attempt:', err);
      }
    };
    
    // Run initialization after a short delay to let JupyterLab bootstrap
    const timer = setTimeout(() => {
      initServiceManager().catch(err => console.error('[JEG-INIT-EFFECT] Error:', err));
    }, 1000);
