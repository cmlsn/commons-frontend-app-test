...
const KernelLifecyclePanel = () => {
  ...
  const openNotebookForKernel = async (kernelRow: KernelRow) => {
    ...
    try {
      ...
      // Try to create notebook file (may fail with 403, but that's ok)
      console.log('[KernelPanel] Attempting to create notebook file...');
      try {
        ...
        if (createResponse.ok || createResponse.status === 201) {
          console.log('[KernelPanel] Notebook file created successfully');
        } else if (createResponse.status === 403) {
          console.log('[KernelPanel] File creation returned 403 (forbidden), but will try session anyway');
        } else {
          console.log('[KernelPanel] File creation returned', createResponse.status);
        }
      } catch (createErr) {
        console.log('[KernelPanel] File creation error (will continue):', createErr);
      }

      // Create session binding this notebook to EXISTING kernel
      const sessionPayload = {
        path: notebookPath,
        type: 'notebook',
        name: '',
        kernel: { id: kernelRow.kernelId },
      };
      
      console.log('[KernelPanel] Session payload:', JSON.stringify(sessionPayload, null, 2));
      console.log('[KernelPanel] Kernel ID:', kernelRow.kernelId);
      ...
      console.log('[KernelPanel] Session response status:', sessionResponse.status);
      console.log('[KernelPanel] Session response body:', JSON.stringify(sessionBody, null, 2));
      
      if (!sessionResponse.ok) {
        const errorMsg = sessionBody?.error || sessionBody?.message || JSON.stringify(sessionBody) || sessionResponse.status;
        console.error('[KernelPanel] Session creation failed:', {
          status: sessionResponse.status,
          body: sessionBody,
          errorMsg,
        });
        throw new Error(`Session create failed: ${errorMsg}`);
      }

      console.log('[KernelPanel] Session created successfully:', sessionBody);
      
      // Step 3: Open the notebook in JupyterLab
      // Try to use JupyterLab's command API if available
      try {
        const app = (window as any).jupyterlab || (window as any).jupyterapp;
        if (app?.commands) {
          console.log('[KernelPanel] Attempting to open notebook via JupyterLab commands');
          await app.commands.execute('docmanager:open', { path: notebookPath });
        }
      } catch (e) {
        console.error('[KernelPanel] Could not auto-open notebook:', e);
      }
      
      setNotice(`✓ Notebook "${notebookPath}" created and bound to kernel. It should open automatically!`);
      await fetchKernelState();
